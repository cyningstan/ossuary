...
void bit_print (Bitmap *bitmap, int x, int y, char *message)
{
    ...
    int v; /* value of byte in character */
    int *d; /* pointer to destination byte on screen */
    ...
    for (b = 0; message[b]; ++b)
        for (r = 0; r < 8; ++r) {
            d = (int *) (bitmap->pixels + 2 * b + x / 4 +
                (y + r) * (bitmap->width / 4));
            v = bitmap->font->pixels[2 * r + 16 * (message[b] -
                bitmap->font->first)] + 0x100 * bitmap->font->pixels[1 +
                2 * r + 16 * (message[b] - bitmap->font->first)];
            if (bitmap->ink != 3 || bitmap->paper != 0)
                v = (v & 0x5555 * bitmap->ink)
                    | ((0xffff ^ v) & 0x5555 * bitmap->paper);
            *d = v;
        }
}
