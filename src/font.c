 ...
 * Font Module.
 *
 * Copyright (C) Damian Walker 2020.
 * Created 29-Jun-2020.
 */
...
Font *fnt_create (int first, int last)
{
    ...
    if (! (font = malloc (sizeof (Font))))
        return NULL;
    if (! (font->pixels = malloc (16 * (last - first + 1)))) {
        free (font);
        return NULL;
    }
...
Font *fnt_copy (Font *src)
{
    ...
    if (! (dst = malloc (sizeof (Font))))
        return NULL;
    if (! (dst->pixels = malloc (16 * (src->last - src->first + 1)))) {
        free (dst);
        return NULL;
    }
    ...
    dst->first = src->first;
    dst->last = src->last;
    memcpy (dst->pixels, src->pixels, 16 * (src->last - src->first + 1));
...
Font *fnt_read (FILE *input)
{
    ...
    if (! (font = malloc (sizeof (Font))))
        return NULL;
    if (! (font->pixels = malloc (16 * (l - f + 1)))) {
        free (font);
        return NULL;
    }

    /* attempt to read the pixel data */
    if (! fread (font->pixels, 16 * (l - f + 1), 1, input)) {
        free (font->pixels);
        free (font);
        return NULL;
    }
...
void fnt_write (Font *font, FILE *output)
{
    ...
    if (! fwrite (&font->last, 1, 1, output))
        return;
    if (! fwrite (font->pixels, 16 * (font->last - font->first + 1), 1,
        output))
        return;
}
...
{
    memcpy (dst->pixels + 16 * (ch - dst->first), src->pixels, 16);
}
...
{
    memcpy (dst->pixels, src->pixels + 16 * (ch - src->first), 16);
}
...
void fnt_colours (Font *font, int ink, int paper)
{
    ...
    int p; /* paper mask */
    int i; /* ink mask */
    int b; /* background of recoloured character */
    int f; /* foreground of recoloured character */
    ...
    for (ch = 0; ch < font->last - font->first; ++ch)
        for (r = 0; r < 15; ++r) {
            f = font->pixels[16 * ch + r] & i;
            b = 0xff ^ font->pixels[16 * ch + r] & p;
            font->pixels[16 * ch + r] = i | p;
        }
}
